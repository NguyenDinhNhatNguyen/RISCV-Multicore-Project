...
The system features a **2x3 Crossbar Interconnect** with Round-Robin arbitration and an **Inter-Core Controller (ICC)** that provides an Atomic Hardware Mutex. The firmware is written in C++ (Bare-metal), utilizing a Spinlock mechanism to ensure safe synchronized access to shared memory.
...
* **Execution Time ($T_{exec}$):** At $F_{max} = 32.32$ MHz ($T_{clk} \approx 30.94$ ns), a synchronization sequence of 5,000 parallel instructions
...
## 📊 Simulation Results
### 1. Crossbar Arbitration (Bus Contention)
When both Core 0 and Core 1 attempt to access the same Shared RAM simultaneously, the Crossbar Arbiter safely stalls Core 1 (`m1_stall` goes HIGH) to grant access to Core 0.

### 2. Hardware Mutex Synchronization (Spinlock)
Core 0 successfully acquires the lock (`mutex_lock` = 1) to enter the critical section. Meanwhile, Core 1 is forced into a software spinlock, repeatedly polling the ICC address until the lock is released.
